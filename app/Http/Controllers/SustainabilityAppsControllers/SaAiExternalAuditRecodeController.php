...
class SaAiExternalAuditRecodeController extends Controller
{
    ...
    public function updateStatusToApproved(string $id)
    {
        ...
        $record = $this->externalAuditInterface->findById($id);

        if (! $record) {
            return response()->json([
                'message' => 'external audit record not found.',
            ], 404);
        }

        $updated = $this->externalAuditInterface->update($id, [
            'status' => 'Approved',
        ]);

        if (! $updated) {
            return response()->json([
                'message' => 'Failed to approve the external audit record.',
            ], 500);
        }
        ...
        return response()->json([
            'message' => 'External audit record approved successfully!',
            'record'  => $record,
        ], 200);
    }
